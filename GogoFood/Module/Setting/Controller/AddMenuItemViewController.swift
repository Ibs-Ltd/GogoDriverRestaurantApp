...
class AddMenuItemViewController: BaseViewController<ProductData>, ImagePickerDelegate {
    ...
    @IBOutlet weak var lbl_disCountItem: UILabel!
    @IBOutlet weak var lbl_itmeImage: UILabel!
    @IBOutlet weak var tmpObj: UITextField!
    ...
    private var disountType = 0 // 0 - none, 1 - percentage, 2 - amount
    private let discountItemName = ["None".localized(), "Percent".localized(), "Amount".localized()]
    ...
    override func viewDidLoad() {
        ...
        lbl_disCountItem.text = "Discount Items".localized()
        lbl_itmeImage.text = "Item Image".localized()
        initView()
        self.createNavigationLeftButton(NavigationTitleString.addMenuItem.localized())
        fetchOptionData()
        // Do any additional setup after loading the view.
    }
    
    func initView() {
        itemName.initiateView(type: .qwertyKeyBoardType, infoText: AppStrings.itemName.localized(), placeHolder: AppStrings.itemName)
        itemName.voidCallBack = {}
        itemCategory.initiateView(type: .dropDown, infoText: AppStrings.category.localized(), placeHolder: AppStrings.selectCategory)
        recommend.initiateView(type: .dropDown, infoText: AppStrings.itemInfo.localized(), placeHolder: AppStrings.selectRecommend.localized())
        itemPrice.initiateView(type: .decimalPad, infoText: AppStrings.itemPrice.localized(), placeHolder: AppStrings.itemPrice)
        itemPrice.voidCallBack = {}
        recommend.initiateView(type: .dropDown, infoText: AppStrings.recommends.localized(), placeHolder: AppStrings.selectRecommend.localized())
        
        discountPercentage.initiateView(type: .numPad, infoText: AppStrings.discountPercentage.localized(), placeHolder: AppStrings.enterPercentage)
        discountPercentage.voidCallBack = {}
        couponCode.initiateView(type: .withoutImage, infoText: AppStrings.couponCode.localized(), placeHolder: AppStrings.enterCouponCode)
        ...
        self.itemPrice.setDollarPrefix()
    }
    ...
    func setDiscountType(){
        self.discount.forEach({ (btn) in
            ...
            if btn.tag == 2{
                discountPercentage.initiateView(type: .numPad, infoText: AppStrings.discountAmount, placeHolder: AppStrings.enterAmount)
            }else if !(self.disountType == 2){
    ...
    func setViewForExtra() {
        extra.initiateView(type: .withoutImage, infoText: AppStrings.extra.localized(), placeHolder: AppStrings.extraPlaceholderTExt)
        extra.voidCallBack = {
...
//            }
    ...
    func setProductData() {
        ...
        for i in 0..<product.dish_images.count {
            ServerImageFetcher.i.loadImageIn(productImageViews[i], url: product.dish_images[i])
        }
        //ServerImageFetcher.i.loadImageIn(productImageViews.first!, url: product.dish_images[0])
    ...
    func setSaveButton() {
        ...
        self.saveButton.voidCallBack = {
            if self.isDataValid() {
                ...
                data.name = self.itemName.getText()
                ...
                data.discount_type = self.disountType == 0 ? "none" : self.disountType == 1 ? "percentage" : "amount"
                ...
                self.repo.modifyProductList(withAdd: (self.product == nil), data: data, productImagesArray: self.productImageViews,onComplition: { (_) in
