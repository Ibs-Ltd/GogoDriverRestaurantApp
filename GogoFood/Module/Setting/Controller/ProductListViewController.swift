...
class ProductListViewController: BaseTableViewController<ProductData> {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.createNavigationLeftButton(NavigationTitleString.listFood.localized())
    ...
    override func viewWillAppear(_ animated: Bool) {
     ...
     func createNavigationRightButton() {
        let b = UIBarButtonItem(title: "ADD".localized(), style: .plain, target: self, action: #selector(navigateToAddMenuItem))
...
class RestaurantProductTableViewCell: BaseTableViewCell<ProductData> {
    ...
    override func initView(withData: ProductData) {
        ...
        if withData.dish_images.count > 0{
            ServerImageFetcher.i.loadImageIn(productImage, url: withData.dish_images[0])
        }else{
            productImage.image = UIImage(named: "restaurant")
        }
        productName.text = withData.name ?? ""
