...
class ResturantTimeViewController: BaseViewController<RestaurantProfileData>, UITextViewDelegate {
    ...
    private let weekDaysName = [AppStrings.mon.localized(), AppStrings.tue.localized(), AppStrings.wed.localized(), AppStrings.thu.localized(), AppStrings.fri.localized(), AppStrings.sat.localized(), AppStrings.sun.localized().localized()]
...
private let repo = SettingRepository()
    ...
    override func viewDidLoad() {
        ...
        setNavigationTitleTextColor(NavigationTitleString.restaurantOpeningTime.localized())
    ...
    fileprivate func setupTranslation(){
        alwaysOpenCheckButton.setTitle("Always Open".localized(), for: .normal)
        selectedHourCheckButton.setTitle("Open Onselected Hour".localized(), for: .normal)
    }
    
    func initView(){
        setViewForSelectedHour()
        // for set estimate Time
        estimateCookingTime.initiateView(type: .largeTimeSelection, infoText: AppStrings.estimateCooking.localized(), placeHolder: "00 min")
